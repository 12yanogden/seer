...
use crate::source::source_strategies::source_strategy_factory::SourceStrategyFactory;

pub fn get_source(inputs: &ArgMatches, pipe: &str, dto: &mut DTO) {
    let source_strategy = SourceStrategyFactory::make(inputs, pipe);
    source_strategy.get_source(dto);
}
...
///
/// ```
/// use seer::{init, get_searchable};
/// let matches = init().try_get_matches_from(vec!["seer", "--exact", "foo", "--text", "bar"]);
...
mod get_searchable_tests {
    ...
    fn test_get_searchable_with_text_option() {
        let matches = init()
            .try_get_matches_from(vec!["seer", "--exact", "foo", "--text", "some text"])
            .unwrap();
        let pipe = None;
    ...
    fn test_get_searchable_with_file_option() {
        ...
        let matches = init()
            .try_get_matches_from(vec!["seer", "--exact", "foo", "--file", file_path])
            .unwrap();
        let pipe = None;
        let searchable = get_searchable(&matches, &pipe).unwrap();
    ...
    fn test_get_searchable_with_piped_input() {
        let matches = init()
            .try_get_matches_from(vec!["seer", "--exact", "foo"])
            .unwrap();
